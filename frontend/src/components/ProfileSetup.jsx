...
function ProfileSetup({ onComplete }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.warn('Profile setup API error, proceeding with calculated targets:', err);
      const calculatedTargets = {
        body_type: bodyType,
        weight_kg: Number(weight),
        height_cm: height ? Number(height) : 175,
        age: age ? Number(age) : 25,
        gender,
        daily_calorie_target: bodyType === 'ectomorph' ? 2400 : bodyType === 'endomorph' ? 1850 : 2100,
        daily_protein_target: Math.round(Number(weight) * (bodyType === 'ectomorph' ? 2.0 : 1.7)),
        daily_carbs_target: 240,
        daily_fats_target: 65,
      };
      onComplete(calculatedTargets);
    } finally {
      setIsLoading(false);
    }
  };
